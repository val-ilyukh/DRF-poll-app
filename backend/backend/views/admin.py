...
class AdminAPIView(APIView):
    '''
    Определяем аутентификацию и разрешения для администратора системы
    '''
...
class AdminPolls(AdminAPIView):
    ...
    def post(self, request):
        try:
            data = request.data             
            #Если нам передали пустую дату, устанавливаем сегодняшнюю                                            
            if data['startDate'] == '':
                data['startDate'] = date.today().strftime('%Y-%m-%d')                     
            s = PollSerializer(data=data)
...
class AdminPollById(AdminAPIView):
    '''
    Выводим детальную информацию по опросу
    '''
...
class AdminQuestionById(AdminAPIView):
        ...
        try:
            ...
            if question.hasOptionType:
                ...
                for option in question.option_set.all():
                    result['options'].append(OptionSerializer(option).data)
            return Response(result)
    ...
    def patch(self, request, pollId, questionId):
        try:
            '''
            Если какой-то пользователь уже ответил на вопрос,
            запрещаем его изменять
            '''
            is_answered = UserResponse.objects.filter(question = Question.objects.get(id = questionId)).count() > 0
            if is_answered:
                raise Exception('You should not change this question, because some users have already answered it')
