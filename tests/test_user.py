...
def createPoll(pollData):
    ...
    for question in pollData['questions']:
        ...
        assert res.status_code == 200
    res2 = requests.get(API_URL + '/polls')
    print('res 2 {}'.format(res2.json()))
    print('poll id = {}'.format(poll_id))
    return poll_id
...
class TestUser:
    def test_available_polls(self, textPollId, choicePollId, completedPollId):
        res = requests.get(API_URL + '/polls', auth=basicAuth)
        assert res.status_code == 200
        polls = res.json()
        print('polls {}'.format(polls)) 
        pollIds = [p['id'] for p in polls]
        print('text poll id {}, pollids {}'.format(textPollId, pollIds))
